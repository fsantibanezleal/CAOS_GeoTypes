...
```bash
pip install geotypes[fast,attr]   # offline; the pure core alone suffices in Pyodide
```
...
```python
import numpy as np
from geotypes import generate_warren_root_ensemble
...
```python
from geotypes import log_resample, bourdet_derivative  # building blocks
from geotypes.preprocess import prepare_curves          # the one-call version
...
```python
from geotypes import dtw_matrix, select_k, pam_kmedoids
...
```python
from geotypes import build_catalogue, Catalogue
...
```python
from geotypes import ConformalAssigner, nearest_medoid
...
```python
import numpy as np
from geotypes.attribute import attribute_geotypes
...
```python
# inside a Pyodide worker: micropip.install("geotypes") or ship the wheel
from geotypes import Catalogue, ConformalAssigner
from geotypes.preprocess import log_resample, second_log_derivative, normalize
