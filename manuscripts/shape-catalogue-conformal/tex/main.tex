...
\newcommand{\versiondoi}{10.5281/zenodo.22832062}
\newcommand{\conceptdoi}{10.5281/zenodo.21511918}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_GeoTypes}
...
CAOS open-research programme \quad$\cdot$\quad CAOS\_GeoTypes, the \texttt{pygeotypes} shape-catalogue package\\[2pt]
...
\thanks{The package \texttt{pygeotypes} is available on PyPI (\url{https://pypi.org/project/pygeotypes});
source code (MIT) at \url{https://github.com/fsantibanezleal/CAOS_GeoTypes}. This note describes a reusable software
...
{Santib\'a\~nez-Leal: pygeotypes, a shape-catalogue library with conformal type assignment}
...
requested confidence, and an empty set flags the signal as out of catalogue. On a reproducible pressure-transient
benchmark the marginal empirical coverage tracks or exceeds the nominal $1-\alpha$ target across the operating
range (0.960 at $\alpha{=}0.05$, 0.938 at $0.10$, 0.885 at $0.15$, 0.800 at $0.20$); per class, one of the two
behaviours stays covered at every level while the other falls below target for $\alpha \ge 0.15$. The prediction sets stay
informative (mean size $1.0$ down to $0.7$), and every alien shape tested is flagged out-of-catalogue at all
confidence levels. These results come from a single seeded run and regenerate exactly from the archived script
and artifacts.
\end{abstract}
...
selection~\cite{rousseeuw}, but in mid-2026 the convenient implementations are hard to distribute: scikit-learn-extra
...
decision tool with finite-sample, per-class coverage guarantees~\cite{vovk,angelopoulos} and an
out-of-catalogue flag. This note therefore contributes (i) a reusable, browser-ready implementation of the \gt{}
methodology; and (ii) the conformal assignment layer, validated empirically. The underlying shape-catalogue
methodology is the contribution of Kamel Targhi \emph{et al.}~\cite{geotypes}, cited wherever it is used.
...
built for such a workflow must run the assignment client-side, which rules out native dependencies. And a
characterisation must be able to say ``out of catalogue'' rather than silently snapping an unusual signal
to the nearest prototype. No single existing package covers this combination: the maintained clustering stacks
are either incompatible with a permissive license or non-portable to the browser, and none attaches a
...
class-conditional split-conformal assigner that returns per-type $p$-values, a prediction set, and an
...
prediction~\cite{vovk,angelopoulos} turns this into a decision with a coverage guarantee. Fit on a \emph{calibration} set of
...
follow naturally. A \emph{prediction set} larger than one signals ambiguity between behaviours at the
requested confidence; a singleton is a confident call. An \emph{empty set} is an out-of-catalogue flag:
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.78\textwidth]{fig-coverage.pdf}
...
$1-\alpha$ target, marginally (bold) and per class: the marginal coverage sits on or above the diagonal across
the operating range ($0.960, 0.938, 0.885, 0.800$ at $\alpha=0.05,0.10,0.15,0.20$); per class, \gt{}~0 stays on
or above target at every level, whereas \gt{}~1 falls below it for $\alpha \ge 0.15$ ($0.817$, $0.708$, $0.678$
against $0.85$, $0.80$, $0.70$). \textbf{(b)} The prediction sets stay
...
Three results follow. First, \emph{the marginal coverage holds empirically}: across the operating range the
measured marginal coverage tracks or exceeds the nominal target at every level ($0.960$ vs $0.95$; $0.938$ vs
$0.90$; $0.885$ vs $0.85$; $0.800$ vs $0.80$; $0.738$ vs $0.70$). Per class the picture is split. The smaller
behaviour (\gt{}~0, $152$ of the $320$ calibration curves) is covered at or above target at every level
($0.970$, $0.965$, $0.955$, $0.894$, $0.798$), while \gt{}~1 ($168$ calibration curves) is covered at
$\alpha \le 0.10$ ($0.950$, $0.911$) but falls below target at $\alpha = 0.15$, $0.20$ and $0.30$ ($0.817$,
$0.708$, $0.678$). In this run the class-conditional calibration therefore protects the smaller behaviour, but
the per-class guarantee is not confirmed for \gt{}~1 at the lower confidence levels. Second, \emph{the sets are informative}: mean prediction-set size falls from
...
tested confidence levels ($\alpha$ from $0.05$ to $0.30$), rather than being forced onto the nearest medoid. The
experiment is a single, seeded run; the numbers regenerate exactly from the archived script and artifact.

\section{Architecture and Availability}\label{sec:arch}

{\raggedright\parindent=1em
The library consists of five small pure-NumPy/SciPy modules (\texttt{preprocess}, \texttt{distance},
\texttt{cluster}, \texttt{catalogue}, \texttt{assign}) plus the optional \texttt{attribute} module. Install lanes
keep the core light:\par}

\begin{lstlisting}[language=bash]
# pure core (numpy + scipy)
pip install pygeotypes
# + dtaidistance (fast offline DTW)
pip install pygeotypes[fast]
# + scikit-learn + shap (drivers)
pip install pygeotypes[attr]
\end{lstlisting}
...
from pygeotypes import (dtw_matrix, select_k,
    pam_kmedoids, build_catalogue,
    ConformalAssigner)
D   = dtw_matrix(X, window=10)      # banded DTW
res = pam_kmedoids(D,
    k=select_k(D, range(2,9))["best_k"], seed=0)
cat = build_catalogue(X, t_grid, D, k=res.k,
    dtw_window=10, result=res)
asg = ConformalAssigner(cat).fit(X_cal,
    labels_cal)
out = asg.predict(new_curve, alpha=0.1)
# out.prediction_set, out.out_of_catalogue
\end{lstlisting}
...
license. Source code and the reproducible validation are available at
\url{https://github.com/fsantibanezleal/CAOS_GeoTypes}.

\section{Limitations}\label{sec:limits}

The shape-catalogue methodology is that of Kamel Targhi \emph{et al.}~\cite{geotypes}; this
...
relative to the catalogue's own metric. In the validation run the per-class coverage of \gt{}~1 falls below
target for $\alpha \ge 0.15$; a single seeded run cannot separate finite-sample fluctuation from a systematic
departure from exchangeability, so the class-conditional guarantee is established here in theory and confirmed
empirically for \gt{}~0 at every level and for \gt{}~1 only at $\alpha \le 0.10$. The out-of-catalogue flag
detects shapes far from every medoid under DTW, not every kind of anomaly. The package's validation document and tests state what is and is not guaranteed.
...
split-conformal assignment layer that turns catalogue lookup into a decision with a finite-sample coverage
guarantee and an out-of-catalogue flag. On a reproducible benchmark the marginal coverage holds across the
operating range (per class it holds for \gt{}~0 throughout and for \gt{}~1 at $\alpha \le 0.10$), the
prediction sets stay informative, and alien shapes are flagged. The result is a portable, empirically validated
tool for characterising physical systems by the shape of their response, which reports when a signal does not
fit the catalogue.
...
{\raggedright\parindent=1em
The validation is \path{manuscripts/shape-catalogue-conformal/figures/coverage_validation.py}: it builds the
...
\path{data/coverage_validation.json} and the medoid curves. The figure script reads those artifacts; the
pipeline schematic is a hand-authored theme-aware drawing. Re-running reproduces every number in
Section~\ref{sec:validation} and Figs.~\ref{fig:catalogue}--\ref{fig:coverage} (the run is seeded).\par}
